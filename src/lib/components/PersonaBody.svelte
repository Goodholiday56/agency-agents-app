...
{#if agent}
  ...
  <header class="pb-head">
    ...
    <div class="pb-titles">
      ...
      <span class="pb-cat">
        {#if onCategory}
          <button class="pb-cat-btn" onclick={() => onCategory(agent.category)} title={t("persona.seeAll", { label: corpus.labelOf(agent.category) })}>
            <Pill tone="brand">
  ...
  <div class="pb-body">
    ...
    <div class="pb-persona">
      {#if loading}
        <LoadingState rows={5} label={t("persona.loading")} />
      {:else if bodyHtml}
      ...
      {:else}
        <p class="text-muted">{t("persona.noBody")}</p>
      {/if}
    </div>
  </div>
{/if}
