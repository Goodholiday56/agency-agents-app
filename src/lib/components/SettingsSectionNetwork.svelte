<script lang="ts">
  ...
  let pathStatuses = $derived.by<PathStatus[]>(() => {
    ...
    return [
      {
        label: t("network.pathGithubCom"),
        desc: t("network.pathGithubComDesc"),
        allowed: !paranoid,
      },
      {
        label: t("network.pathApiGithub"),
        desc: t("network.pathApiGithubDesc"),
        allowed: !paranoid,
      },
      {
        label: t("network.pathRaw"),
        desc: t("network.pathRawDesc"),
        allowed: !paranoid,
      },
      {
        label: t("network.pathUpdate"),
        desc: t("network.pathUpdateDesc"),
        allowed: !paranoid,
      },
      {
        label: t("network.pathBrowser"),
        desc: t("network.pathBrowserDesc"),
        allowed: true,
      },
    ];
  });
...
<div class="section">
  <h2>{@html t("network.title")}</h2>

  {#if settings.loading && !settings.data}
    <p class="lead">{t("network.loading")}</p>
  {:else if settings.corruptOnDisk}
    <div class="callout corrupt" role="alert">
      <div class="callout-head">
        <AlertTriangle size={18} />
        <strong>{t("network.corruptHeading")}</strong>
      </div>
      <p class="callout-body">
        {t("network.corruptBody")}
      </p>
      ...
      <button type="button" class="btn-danger" onclick={handleReset} disabled={settings.loading}>
        <RefreshCw size={14} /> {t("network.resetDefaults")}
      </button>
    </div>
  {:else if settings.data}
    <div class="field">
      <label class="toggle" title={t("network.offlineModeDesc")}>
        <input
          type="checkbox"
        ...
        <span class="toggle-label">{t("network.offlineMode")}</span>
      </label>
      <p class="hint" id="offline-mode-hint">{t("network.offlineModeDesc")}</p>
    ...
    <div class="field disclosure">
      <span class="field-label">{t("network.whereConnects")}</span>
      <ol class="paths">
        {#each pathStatuses as p, i (p.label)}
          <li>
            <span class="num">{i + 1}.</span>
            <span class="status" aria-label={p.allowed ? t("network.allowed") : t("network.blocked")}>
      ...
      <p class="hint">{t("network.footerHint")}</p>
    </div>
