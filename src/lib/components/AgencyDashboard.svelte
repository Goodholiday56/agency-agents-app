<script lang="ts">
  ...
  import { t as i18n } from "$lib/stores/i18n.svelte";
  ...
  const healthSegments = $derived([
    { label: i18n("health.inSync"),   value: byState.current,  color: "var(--color-success)", onClick: () => ui.openAgents(null, "current") },
    { label: i18n("health.outdated"),  value: byState.outdated,  color: "var(--color-warning)", onClick: () => ui.openAgents(null, "outdated") },
    { label: i18n("health.modified"),  value: byState.modified,  color: "color-mix(in srgb, var(--color-warning) 55%, var(--color-danger))", onClick: () => ui.openAgents(null, "outdated") },
    { label: i18n("health.untracked"), value: byState.foreign,   color: "var(--color-brand)",   onClick: () => ui.openAgents(null, "foreign") },
    { label: i18n("health.missing"),   value: byState.removed,   color: "var(--color-danger)",  onClick: () => ui.openAgents(null, "removed") },
  ]);
  ...
  const sunburstGroups = $derived([
    { label: i18n("dashboard.global"), value: globalRows.length, color: "var(--color-brand)", onClick: () => ui.setSection("tools"), children: globalByTool },
    { label: i18n("dashboard.projects"), value: projectRows.length, color: "var(--color-success)", onClick: () => ui.setSection("projects"), children: projByProject },
  ]);
...
<section class="dash">
  <div class="stats">
    <button class="stat" onclick={() => ui.openAgents()}>
      ...
      <span class="s-lbl">{i18n("dashboard.agentsAvailable")}</span>
    </button>
    <button class="stat" onclick={() => ui.openAgents()}>
      <span class="s-num">{managed}</span>
      <span class="s-lbl">{i18n("dashboard.totalInstalled")}</span>
      {#if fromOtherTools > 0}
        <span class="s-sub">{trackedByApp} {i18n("dashboard.viaThisApp")} · {fromOtherTools} {i18n("dashboard.otherTools")}</span>
      {/if}
    </button>
    {#if attention > 0}
      <button class="stat warn" onclick={() => ui.openAgents(null, "attention")}>
        <span class="s-num">{attention}</span>
        <span class="s-lbl">{i18n("dashboard.needAttention")}</span>
      </button>
    {/if}
    {#if foreign > 0}
      <button class="stat info" onclick={() => ui.openAgents(null, "foreign")}>
        <span class="s-num">{foreign}</span>
        <span class="s-lbl">{i18n("dashboard.foundToTrack")}</span>
      </button>
    {/if}
  </div>
  ...
  <div class="cols">
    <div class="card">
      <h3 class="c-title">{i18n("dashboard.totalInstalled")}</h3>
      <div class="card-fill center">
        {#if managed === 0}
          <p class="muted">{i18n("dashboard.nothingInstalled")}</p>
        {:else}
    ...
    <div class="card">
      <h3 class="c-title">{i18n("dashboard.projects")}</h3>
      <div class="card-fill">
        {#if projectBreakdown.length === 0}
        <p class="muted">
          {i18n("dashboard.noProjects")}
          <button class="link inline" onclick={() => ui.setSection("projects")}>{i18n("dashboard.openProjects")}</button>
        </p>
      {:else}
        <ul class="proj-list">
            ...
            <li>
              <button class="proj-row" onclick={() => ui.selectProject(p.path)} title={p.path}>
                <span class="proj-top">
                  ...
                  <span class="proj-total">{p.total} {p.total === 1 ? i18n("common.agent") : i18n("common.agents")}</span>
                </span>
                <span class="proj-bar">
                ...
                <span class="proj-divs">
                  ...
                  {/each}
                  {#if p.divisions.length > 4}<span class="proj-div more">{i18n("dashboard.plusMore", { n: p.divisions.length - 4 })}</span>{/if}
                </span>
              </button>
            </li>
          {/each}
  ...
  <div class="cols">
    <div class="card">
      <h3 class="c-title">{i18n("dashboard.installHealth")}</h3>
      <div class="card-fill center">
        {#if totalInstalls === 0}
          <p class="muted">{i18n("dashboard.noHealth")}</p>
        {:else}
    ...
    <div class="card">
      <h3 class="c-title">{i18n("dashboard.coverageByTool")}</h3>
      <div class="card-fill">
        {#if perTool.length === 0}
        <p class="muted">{i18n("dashboard.noCoverage")}</p>
      {:else}
        <ul class="bars">
          {#each perTool as t (t.id)}
            <li>
              <button class="bar-btn" onclick={() => ui.openTools(t.id)} title={t.detected ? i18n("dashboard.detected") : i18n("dashboard.notDetected")}>
      ...
      <button class="link" onclick={() => ui.setSection("tools")}>{i18n("dashboard.manageTools")}</button>
    </div>
  </div>

  <div class="card">
    <h3 class="c-title">{i18n("dashboard.crossToolCoverage")}</h3>
    <CoverageDonuts bind:hovered={divisionHover} />
    <div class="merge-sep">
      <span class="merge-cap">{i18n("dashboard.catalogByDivision")}</span>
    </div>
